...
//  ret = read_reg(hspi1,  LSM6DSL_CTRL1_XL, (uint8_t*)&ctrl1_xl, 1,huart1);
  ...
  if(ret == 0){
   ...
   // ret = lsm6dsl_write_reg(ctx, LSM6DSL_CTRL1_XL, (uint8_t*)&ctrl1_xl, 1);
    ...
    ret = write_reg(hspi1,  LSM6DS3_ACC_GYRO_CTRL1_XL, ( LSM6DS3_ACC_GYRO_BW_XL_100Hz |LSM6DS3_ACC_GYRO_FS_XL_16g|LSM6DS3_ACC_GYRO_ODR_XL_416Hz), 1,huart1);
...
int32_t lsm6dsl_acc_set_fullscale(SPI_HandleTypeDef hspi1  )
{
  ...
  if(ret == 0){
	//ctrl1_xl = (uint8_t) val;
   // ret = lsm6dsl_write_reg(ctx, LSM6DSL_CTRL1_XL, (uint8_t*)&ctrl1_xl, 1);
    ret = write_reg(hspi1,  LSM6DSL_CTRL1_XL, (uint8_t*)LSM6DSL_2g, 1,huart1);
...
int32_t lsm6dsl_gyro_set_data_rate(SPI_HandleTypeDef hspi1  )
{
  ...
  if(ret == 0){
	//ctrl1_xl = (uint8_t) val;
   // ret = lsm6dsl_write_reg(ctx, LSM6DSL_CTRL1_XL, (uint8_t*)&ctrl1_xl, 1);
    ret = write_reg(hspi1,  LSM6DSL_CTRL2_G, (LSM6DS3_ACC_GYRO_FS_G_2000dps | LSM6DS3_ACC_GYRO_ODR_G_416Hz), 1,huart1);
...
int32_t lsm6dsl_gyro_set_fullscale(SPI_HandleTypeDef hspi1  )
{
  ...
  if(ret == 0){
	//ctrl1_xl = (uint8_t) val;
   // ret = lsm6dsl_write_reg(ctx, LSM6DSL_CTRL1_XL, (uint8_t*)&ctrl1_xl, 1);
    ret = write_reg(hspi1,  LSM6DSL_CTRL2_G, (uint8_t*)LSM6DSL_250dps, 1,huart1);
...
{
	int16_t data = 0;
	readRegisterInt16(hspi1, &data,(0x80 | LSM6DS3_ACC_GYRO_OUTX_L_XL));
...
{
	int16_t data = 0;
	readRegisterInt16(hspi1, &data, (0x80 | LSM6DS3_ACC_GYRO_OUTY_L_XL));
	return data;
}
int16_t lsm6dsl_acc_get_raw_z(SPI_HandleTypeDef hspi1  )
{
	int16_t data = 0;
	readRegisterInt16(hspi1, &data,( 0x80 | LSM6DS3_ACC_GYRO_OUTZ_L_XL));
	return data;
}
int16_t lsm6dsl_gyro_get_raw_x(SPI_HandleTypeDef hspi1  )
{
	int16_t data = 10;
	readRegisterInt16(hspi1, &data,( 0x80 | LSM6DS3_ACC_GYRO_OUTX_L_G));
	return data;
}
int16_t lsm6dsl_gyro_get_raw_y(SPI_HandleTypeDef hspi1  )
{
	int16_t data = 0;
	readRegisterInt16(hspi1, &data,( 0x80 | LSM6DS3_ACC_GYRO_OUTY_L_G));
	return data;
}
int16_t lsm6dsl_gyro_get_raw_z(SPI_HandleTypeDef hspi1  )
{
	int16_t data = 0;
	readRegisterInt16(hspi1, &data, ( 0x80 |LSM6DS3_ACC_GYRO_OUTZ_L_G));
	return data;
}
int16_t readRegisterInt16( SPI_HandleTypeDef hspi1, int16_t* dataout, uint8_t reg  )
{
	uint8_t buff[2];
	int16_t output;
	read_reg(hspi1, reg, &buff,2,huart1);
